...
# Parámetros D-H:
#            1        A       2        3       4        5        61      62       71      72       81       82       EF 
d  = [       5,       0,      0,       2,      0,       0,    p[5],   -p[5],       0,      0,       0,      0,       0]
th = [    p[0],    p[1],     90,    p[2],90-p[3],    p[4],       0,       0,       0,      0,       0,      0,       0]
a  = [       0,       2,      0,       0,      0,       0,       0,       0,       1,      1,    p[6],   p[6],  1+p[6]]
al = [      90,       0,     90,      90,     90,       0,      90,      90,       0,      0,       0,      0,       0]
...
T1A=matriz_T(d[1],th[1],a[1],al[1])
T0A=np.dot(T01,T1A)
TA2=matriz_T(d[2],th[2],a[2],al[2])
T02=np.dot(T0A,TA2)
T23=matriz_T(d[3],th[3],a[3],al[3])
T03=np.dot(T02,T23)
T34=matriz_T(d[4],th[4],a[4],al[4])
T04=np.dot(T03,T34)
T45=matriz_T(d[5],th[5],a[5],al[5])
T05=np.dot(T04,T45)

T561=matriz_T(d[6],th[6],a[6],al[6])
T061=np.dot(T05,T561)
T562=matriz_T(d[7],th[7],a[7],al[7])
T062=np.dot(T05,T562)

T6171=matriz_T(d[8],th[8],a[8],al[8])
T071=np.dot(T061,T6171)
T6272=matriz_T(d[9],th[9],a[9],al[9])
T072=np.dot(T062,T6272)

T7181=matriz_T(d[10],th[10],a[10],al[10])
T081=np.dot(T071,T7181)
T7282=matriz_T(d[11],th[11],a[11],al[11])
T082=np.dot(T072,T7282)

T5EF=matriz_T(d[12],th[12],a[12],al[12])
T0EF=np.dot(T05,T5EF)
...
muestra_origenes([o00,o10,o20,o30,o40,o50,[[o61,o71,o81],[o62,o72,o82]]],oef)
muestra_robot   ([o00,o10,o20,o30,o40,o50,[[o61,o71,o81],[o62,o72,o82]]],oef)
